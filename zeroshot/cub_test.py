...
def zsl_test(epoch, net, optimizer, log):
    ...
    # w_attr[:, 0].sum()
    order_cub_attr = order_cub_attr[NUM_SEEN:, :] / 100
    ...
    np.save("data/cub_acc_bin.npy", acc_bin)
...
def gzsl_test(epoch, net, optimizer):
    ...
    USE_GPU = torch.cuda.is_available()
    order_cub_attr = np.load("data/order_cub_attr.npy")
    order_cub_attr[150:, :] = order_cub_attr[150:, :] * 1.5
...
def gzsl_test0(epoch, net, optimizer, log, gamma=2.):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        ...
        seen_prob, seen_class = torch.max(logit[:, :150], 1)
        unseen_prob, unseen_class = torch.max(logit[:, 150:], 1)
