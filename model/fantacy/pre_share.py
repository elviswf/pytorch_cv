...
class CifarPreActResNetPreShare(nn.Module):

        def __init__(self, depth, nb_classes, share_end=1):
            super(CifarPreActResNetPreShare, self).__init__()
            if (depth - 2) % 9 != 0:
                raise Exception("depth should be 9n+2!")

            nb_sub_blocks = (depth - 2) // 9

            self.share_end = share_end
            ...
            if share_end < 1:
                self.clone1 = _make_resnet_block(16, 64, stride=1, nb_sub_blocks=nb_sub_blocks)
            if share_end < 2:
...
### TODO: imagenet share pre-act resnet
class ImagenetPreActResNetShare(nn.Module):
    ...
    def __init__(self, depth=None, nb_classes=1000, blocks=None, share=0):
        super(ImagenetPreActResNetShare, self).__init__()
...
def build_preact_resent_pre_share(depth, nb_classes, mode='cifar', blocks=None, share=0):
    if mode == "cifar":
        return CifarPreActResNetPreShare(depth, nb_classes, share_end=share)
    elif mode == "imagenet":
        return ImagenetPreActResNetShare(depth, nb_classes, blocks=blocks, share=share)
...
def unit_test():
    model = build_preact_resent_pre_share(110, 10, mode='cifar', share=2)
